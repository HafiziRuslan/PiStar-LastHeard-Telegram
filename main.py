    ...
    def __init__(self, logline: str):
        ...
        if match:
            ...
            if self.callsign.isnumeric():
                self.url = (
                    f"https://database.radioid.net/database/view?id={self.callsign}"
                )
            else:
        ...
        if self.destination.startswith("TG "):
            for tg_file in tg_files:
                if os.path.isfile(tg_file):
                    try:
                        with open(
                            tg_file, "r", encoding="UTF-8", errors="replace"
                        ) as file:
                            for line in file:
    ...
    try:
        while not shutdown_flag.is_set():
            try:
                ...
                if parsed_line.timestamp is not None and (
                    last_event is None or parsed_line.timestamp > last_event
                ):
